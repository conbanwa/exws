...
func (bm *Bitmex) doAuthRequest(m, uri, param string, r any) error {
	nonce := time.Now().UTC().Unix() + 3600
	sign := bm.generateSignature(m, uri, param, fmt.Sprint(nonce))
	resp, err := NewRequest(bm.HttpClient, m, bm.Endpoint+uri, param, map[string]string{
		"User-Agent":    "goex/bitmex",
		"Content-Type":  "application/json",
		"Accept":        "application/json",
		"api-expires":   fmt.Sprint(nonce),
		"api-key":       bm.ApiKey,
		"api-signature": sign})
	logs.D("response:", string(resp))
	if err != nil {
		return err
	} else {
		//println(string(resp))
...
func (bm *Bitmex) toJson(param any) string {
...
func (bm *Bitmex) PlaceFutureOrder2(currencyPair CurrencyPair, contractType, price, amount string, openType, matchPrice int, leverRate float64) (*FutureOrder, error) {
	...
	createOrderParameter.Text = "goex/tree/master/bitmex"
	...
	createOrderParameter.OrderQty = num.ToInt[int](amount)
	if matchPrice == 0 {
		createOrderParameter.Price = num.ToFloat64(price)
	} else {
...
func (bm *Bitmex) GetFutureDepth(currencyPair CurrencyPair, contractType string, size int) (*Depth, error) {
	...
	for _, r := range resp {
		...
		case "sell":
			dep.AskList = append(dep.AskList, DepthRecord{Price: num.ToFloat64(rr["price"]), Amount: num.ToFloat64(rr["size"])})
		case "buy":
			dep.BidList = append(dep.BidList, DepthRecord{Price: num.ToFloat64(rr["price"]), Amount: num.ToFloat64(rr["size"])})
		}
	}
	return dep, nil
}
func (bm *Bitmex) GetFutureTicker(currencyPair CurrencyPair, contractType string) (*Ticker, error) {
	...
	tickermap, isok := resp[0].(map[string]any)
	if !isok {
		return nil, fmt.Errorf("response format error [%s]", resp[0])
	}
	date, _ := time.Parse(time.RFC3339, tickermap["timestamp"].(string))
	return &Ticker{
		Pair: currencyPair,
		Last: num.ToFloat64(tickermap["lastPrice"]),
		High: num.ToFloat64(tickermap["highPrice"]),
		Low:  num.ToFloat64(tickermap["lowPrice"]),
		Vol:  num.ToFloat64(tickermap["homeNotional24h"]),
		Sell: num.ToFloat64(tickermap["askPrice"]),
		Buy:  num.ToFloat64(tickermap["bidPrice"]),
...
func (bm *Bitmex) GetIndicativeFundingRate(symbol string) (float64, *time.Time, error) {
	...
	retmap, isok := resp[0].(map[string]any)
	if !isok {
		return 0, nil, fmt.Errorf("response format error [%s]", resp[0])
	}
	t, _ := time.Parse(time.RFC3339, retmap["fundingTimestamp"].(string))
	return num.ToFloat64(retmap["indicativeFundingRate"]), &t, nil
}
	...
	for _, record := range response {
		...
		klines = append(klines, FutureKline{
			Kline: &Kline{
				Timestamp: t.Unix(),
				Pair:      currency,
				Open:      num.ToFloat64(r["open"]),
				High:      num.ToFloat64(r["high"]),
				Low:       num.ToFloat64(r["low"]),
				Close:     num.ToFloat64(r["close"]),
				Vol:       num.ToFloat64(r["volume"])}})
	}
	return klines, nil
}
func (bm *Bitmex) GetTrades(contract_type string, currency CurrencyPair, since int64) ([]Trade, error) {
	...
	if since/int64(time.Second) != 1 {
		sinceTime = time.Unix(since/int64(time.Second), 0).UTC()
	}
	...
	for _, v := range response {
		...
		trades = append(trades, Trade{
			Tid:    num.ToInt[int64](vv["trdMatchID"]),
			Type:   side,
			Amount: num.ToFloat64(vv["size"]),
			Price:  num.ToFloat64(vv["price"]),
			Date:   timestamp.Unix(),
			Pair:   currency,
		})
	}
