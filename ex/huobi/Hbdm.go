...
func hbdmInit() {
	go func() {
		defer func() {
			logs.I("[hbdm] Get Futures Tick Size Finished.")
		}()
		...
		for {
			...
			case <-intervalTimer.C:
				...
				if err != nil {
					logs.E("[hbdm] get contract info error=", err)
					goto reset
				}
...
func (dm *Hbdm) GetFuturePosition(currencyPair CurrencyPair, contractType string) ([]FuturePosition, error) {
	...
	for _, d := range data {
		...
		pos.ContractId = num.ToInt[int64](d.ContractCode[3:])
		pos.Symbol = currencyPair
		switch d.Direction {
		case "buy":
			//positions = append(positions, FuturePosition{
			//	ContractType:  d.ContractType,
			//	ContractId:    int64(num.ToInt[int](d.ContractCode[3:])),
			//	Symbol:        currencyPair,
			//	BuyAmount:     d.Volume,
...
func (dm *Hbdm) FutureCancelOrder(currencyPair CurrencyPair, contractType, orderId string) (bool, error) {
	...
	if len(data.Errors) > 0 {
		return false, fmt.Errorf("%d:[%s]", data.Errors[0].ErrCode, data.Errors[0].ErrMsg)
	} else {
		return true, nil
	}
}
...
func (dm *Hbdm) GetFutureEstimatedPrice(currencyPair CurrencyPair) (float64, error) {
	...
	if ret["status"].(string) != "ok" {
		return -1, fmt.Errorf("%+v", ret)
	}
	return num.ToFloat64(ret["data"].(map[string]any)["delivery_price"]), nil
}
func (dm *Hbdm) GetFutureTicker(currencyPair CurrencyPair, contractType string) (*Ticker, error) {
	...
	s := ret["status"].(string)
	if s == "error" {
		log.Println(ret["err-code"])
		return nil, errors.New(ret["err-msg"].(string))
	}
	tick, ok1 := ret["tick"].(map[string]any)
	ask, ok2 := tick["ask"].([]any)
	bid, ok3 := tick["bid"].([]any)
	if !ok1 || !ok2 || !ok3 {
		return nil, errors.New("no tick data")
	}
	return &Ticker{
		Pair: currencyPair,
		Last: num.ToFloat64(tick["close"]),
		Vol:  num.ToFloat64(tick["amount"]),
		Low:  num.ToFloat64(tick["low"]),
		High: num.ToFloat64(tick["high"]),
		Sell: num.ToFloat64(ask[0]),
		Buy:  num.ToFloat64(bid[0]),
		Date: num.ToInt[uint64](ret["ts"])}, nil
}
func (dm *Hbdm) GetFutureDepth(currencyPair CurrencyPair, contractType string, size int) (*Depth, error) {
	symbol := dm.adaptSymbol(currencyPair, contractType)
	symUrl := dm.config.Endpoint + "/market/depth?type=step0&symbol=" + symbol
	ret, err := HttpGet(dm.config.HttpClient, symUrl)
	if err != nil {
		return nil, err
	}
	s := ret["status"].(string)
	...
	mills := num.ToInt[uint64](ret["ts"])
	dep.UTime = time.Unix(int64(mills/1000), int64(mills%1000)*int64(time.Millisecond))
	tick, ok1 := ret["tick"].(map[string]any)
	asks, ok2 := tick["asks"].([]any)
	bids, ok3 := tick["bids"].([]any)
	if !ok1 || !ok2 || !ok3 {
		return nil, errors.New("data error")
	}
	for _, item := range asks {
		askItem := item.([]any)
		dep.AskList = append(dep.AskList, DepthRecord{Price: num.ToFloat64(askItem[0]), Amount: num.ToFloat64(askItem[1])})
	}
	for _, item := range bids {
		bidItem := item.([]any)
		dep.BidList = append(dep.BidList, DepthRecord{Price: num.ToFloat64(bidItem[0]), Amount: num.ToFloat64(bidItem[1])})
	}
...
func (dm *Hbdm) GetFutureIndex(currencyPair CurrencyPair) (float64, error) {
	...
	datamap := ret["data"].([]any)
	index := datamap[0].(map[string]any)["index_price"]
	return num.ToFloat64(index), nil
}
func (dm *Hbdm) GetKlineRecords(contract_type string, currency CurrencyPair, period KlinePeriod, size int, opt ...OptionalParameter) ([]FutureKline, error) {
	...
	symUrl := fmt.Sprintf("%s/market/history/kline?symbol=%s&period=%s&size=%d", dm.config.Endpoint, symbol, periodS, size)
	var ret struct {
		BaseResponse
		Data []struct {
			Id     int64   `json:"id"`
	...
	err := HttpGet4(dm.config.HttpClient, symUrl, nil, &ret)
	if err != nil {
		return nil, err
	}
	if ret.Status != "ok" {
...
func (dm *Hbdm) doRequest(path string, params *url.Values, data any) error {
	dm.buildPostForm("POST", path, params)
	jsonD := slice.ValuesToJson(*params)
...
func (dm *Hbdm) formatPriceSize(contract string, currency Currency, price string) string {
	...
	return num.FloatToString(num.ToFloat64(price), math.Pow10(-tickSize))
}
