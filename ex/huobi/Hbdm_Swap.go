...
func (swap *HbdmSwap) String() string {
	return cons.HBDM_SWAP
}
func (swap *HbdmSwap) GetFutureTicker(currencyPair cons.CurrencyPair, contractType string) (*Ticker, error) {
	tickerUrl := fmt.Sprintf("%s%s?contract_code=%s", swap.base.config.Endpoint, tickerApiPath, currencyPair.ToSymbol("-"))
	responseBody, err := web.HttpGet5(swap.base.config.HttpClient, tickerUrl, map[string]string{})
	if err != nil {
		return nil, err
	}
...
func (swap *HbdmSwap) GetFutureDepth(currencyPair cons.CurrencyPair, contractType string, size int) (*Depth, error) {
	step := 0
	if size <= 20 {
		step = 6
	}
	depthUrl := fmt.Sprintf("%s%s?contract_code=%s&type=step%d", swap.base.config.Endpoint, marketApiPath, currencyPair.ToSymbol("-"), step)
	responseBody, err := web.HttpGet5(swap.base.config.HttpClient, depthUrl, map[string]string{})
	if err != nil {
		return nil, err
	}
	logs.Debugf("response body: %s", string(responseBody))
	...
	for i, item := range tickResponse.Tick.Bids {
		...
		dep.BidList = append(dep.BidList, DepthRecord{
			Price:  num.ToFloat64(item[0]),
			Amount: num.ToFloat64(item[1]),
		})
	}
...
func (swap *HbdmSwap) GetFutureUserinfo(currencyPair ...cons.CurrencyPair) (*FutureAccount, error) {
	...
	futureAccount.FutureSubAccounts = make(map[cons.Currency]FutureSubAccount, 4)
	for _, acc := range accountInfoResponse {
		currency := cons.NewCurrency(acc.Symbol, "")
...
func (swap *HbdmSwap) PlaceFutureOrder(currencyPair cons.CurrencyPair, contractType, price, amount string, openType, matchPrice int, leverRate float64) (string, error) {
	param := url.Values{}
...
func (swap *HbdmSwap) LimitFuturesOrder(currencyPair cons.CurrencyPair, contractType, price, amount string, openType int, opt ...cons.LimitOrderOptionalParameter) (*FutureOrder, error) {
...
func (swap *HbdmSwap) MarketFuturesOrder(currencyPair cons.CurrencyPair, contractType, amount string, openType int) (*FutureOrder, error) {
...
func (swap *HbdmSwap) FutureCancelOrder(currencyPair cons.CurrencyPair, contractType, orderId string) (bool, error) {
	param := url.Values{}
...
func (swap *HbdmSwap) GetFuturePosition(currencyPair cons.CurrencyPair, contractType string) ([]FuturePosition, error) {
	param := url.Values{}
	param.Set("contract_code", currencyPair.ToSymbol("-"))
	var (
	...
	for _, pos := range positionResponse {
		...
		case "sell":
			...
			tempPositionMap[pos.ContractCode].Symbol = cons.NewCurrencyPair3(pos.ContractCode, "-")
...
func (swap *HbdmSwap) GetFutureOrders(orderIds []string, currencyPair cons.CurrencyPair, contractType string) ([]FutureOrder, error) {
	return nil, nil
}
func (swap *HbdmSwap) GetFutureOrder(orderId string, currencyPair cons.CurrencyPair, contractType string) (*FutureOrder, error) {
	var (
...
func (swap *HbdmSwap) GetFutureOrderHistory(pair cons.CurrencyPair, contractType string, optional ...OptionalParameter) ([]FutureOrder, error) {
	params := url.Values{}
	params.Add("status", "0")     //all
	params.Add("type", "1")       //all
	params.Add("trade_type", "0") //all
	if contractType == "" || contractType == cons.SWAP_CONTRACT {
...
func (swap *HbdmSwap) GetUnfinishFutureOrders(currencyPair cons.CurrencyPair, contractType string) ([]FutureOrder, error) {
	param := url.Values{}
	param.Set("contract_code", currencyPair.ToSymbol("-"))
...
func (swap *HbdmSwap) GetContractValue(currencyPair cons.CurrencyPair) (float64, error) {
	switch currencyPair {
	case cons.BTC_USD, cons.BTC_USDT:
		return 100, nil
	default:
		return 0, nil
	}
}
func (swap *HbdmSwap) GetKlineRecords(contractType string, currency cons.CurrencyPair, period cons.KlinePeriod, size int, opt ...OptionalParameter) ([]FutureKline, error) {
	...
	responseBody, err := web.HttpGet5(swap.base.config.HttpClient, apiUrl, map[string]string{})
	if err != nil {
		return nil, err
	}
	logs.Debugf("response body: %s", string(responseBody))
...
func adaptKlinePeriodForOKEx(period int) string {
	switch period {
	case cons.KLINE_PERIOD_1MIN:
		return "1min"
	case cons.KLINE_PERIOD_5MIN:
		return "5min"
	case cons.KLINE_PERIOD_15MIN:
		return "15min"
	case cons.KLINE_PERIOD_30MIN:
		return "30min"
	case cons.KLINE_PERIOD_1H:
		return "1hour"
	case cons.KLINE_PERIOD_4H:
		return "4hour"
	case cons.KLINE_PERIOD_1DAY:
		return "day"
	case cons.KLINE_PERIOD_2H:
		return "2hour"
	case cons.KLINE_PERIOD_1WEEK:
		return "week"
	default:
		return "day"
	}
...
func (swap *HbdmSwap) GetTrades(contractType string, currencyPair cons.CurrencyPair, since int64) ([]q.Trade, error) {
	panic("not implement")
}
func (swap *HbdmSwap) GetFee() (float64, error) {
	panic("not implement")
}
func (swap *HbdmSwap) GetFutureIndex(currencyPair cons.CurrencyPair) (float64, error) {
	panic("not implement")
}
func (swap *HbdmSwap) GetDeliveryTime() (int, int, int, int) {
	panic("not implement")
}
func (swap *HbdmSwap) GetFutureEstimatedPrice(currencyPair cons.CurrencyPair) (float64, error) {
	panic("not implement")
}
