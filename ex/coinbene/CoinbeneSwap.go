...
func (swap *CoinbeneSwap) String() string {
	return COINBENE
}
func (swap *CoinbeneSwap) GetFutureTicker(currencyPair CurrencyPair, contractType string) (*Ticker, error) {
	...
	return &Ticker{
		Pair: currencyPair,
		Last: num.ToFloat64(tick.LastPrice),
		Sell: num.ToFloat64(tick.BestAskPrice),
		Buy:  num.ToFloat64(tick.BestBidPrice),
		Low:  num.ToFloat64(tick.Low24h),
		High: num.ToFloat64(tick.High24h),
		Vol:  num.ToFloat64(tick.Volume24h)}, nil
}
...
func (swap *CoinbeneSwap) GetFutureOrder(orderId string, currencyPair CurrencyPair, contractType string) (*FutureOrder, error) {
	...
	json.Unmarshal(resp.Data, &data)
	if data.OrderId == "" {
		return nil, fmt.Errorf("not fund order[%s]", orderId)
	}
	return &FutureOrder{
...
func (swap *CoinbeneSwap) GetKlineRecords(contractType string, currency CurrencyPair, period KlinePeriod, size int, opt ...OptionalParameter) ([]FutureKline, error) {
	panic("")
}
func (swap *CoinbeneSwap) GetTrades(contractType string, currencyPair CurrencyPair, since int64) ([]Trade, error) {
	panic("")
}
func (swap *CoinbeneSwap) GetFutureEstimatedPrice(currencyPair CurrencyPair) (float64, error) {
	panic("")
}
func (swap *CoinbeneSwap) doAuthRequest(method, uri string, param any) (*baseResp, error) {
	...
	resp, err := NewRequest(swap.config.HttpClient, method, swap.config.Endpoint+uri, postBody, header)
	if err != nil {
		return nil, err
	}
	//println(string(resp))
	...
	if ret.Code != 200 {
		return nil, fmt.Errorf("[%d]%s", ret.Code, ret.Message)
	}
	return &ret, nil
}
