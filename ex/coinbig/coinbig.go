...
func (cb *CoinBig) GetAccount() (*Account, error) {
	...
	body, err := web.HttpPostForm(cb.httpClient, api_url, params)
	if err != nil {
		return nil, err
	}
	...
	balances, isok := bodyDataMap["data"].(map[string]any)
	if isok != true {
		return nil, errors.New("no account data")
	}
	info, isok := balances["info"].(map[string]any)
	if isok != true {
		return nil, errors.New("no account data")
	}
	acc := Account{}
	acc.Exchange = cb.String()
	acc.SubAccounts = make(map[Currency]SubAccount)
	free := info["free"].(map[string]any)
	//log.Println(free)
	for k, v := range free {
	...
	freezed := info["freezed"].(map[string]any)
...
func (cb *CoinBig) placeOrder(amount, price string, pair CurrencyPair, orderType, orderSide string) (*Order, error) {
	...
	data, isok := bodyDataMap["data"].(map[string]any)
	if isok != true {
		return nil, errors.New("no order information")
	}
	orderId := num.ToInt[int](data["order_id"])
	side := BUY
	if orderSide == "sell" {
		side = SELL
	}
...
func (cb *CoinBig) CancelOrder(orderId string, currencyPair CurrencyPair) (bool, error) {
	...
	cb.buildSigned(&params)
	resp, err := web.HttpPostForm(cb.httpClient, path, params)
...
func (cb *CoinBig) GetOneOrder(orderId string, currencyPair CurrencyPair) (*Order, error) {
	...
	data, _ := bodyDataMap["data"].(map[string]any)
	//list, _ := data["orders"].([]any)
	result, _ := data["result"].(bool)
	if result != true {
		return nil, errors.New("no order info")
	}
	vv := data["orders"].(map[string]any)
	return &Order{
		Currency:   currencyPair,
		OrderID2:   vv["order_id"].(string),
		Price:      num.ToFloat64(vv["price"]),
		Amount:     num.ToFloat64(vv["count"]),
		DealAmount: num.ToFloat64(vv["count"]) - num.ToFloat64(vv["leftCount"]),
		Side:       orderTypeAdapter(vv["isLimit"].(int), vv["entrustType"].(int)),
		Status:     orderStatusAdapter(vv["status"].(int)),
		OrderTime:  num.ToInt[int](vv["create_date"]),
	}, nil
}
func (cb *CoinBig) GetUnfinishedOrders(currencyPair CurrencyPair) ([]Order, error) {
...
func (cb *CoinBig) GetDepth(size int, currencyPair CurrencyPair) (*Depth, error) {
	...
	bodyDataMap, err := web.HttpGet(cb.httpClient, path)
	...
	data := bodyDataMap["data"].(map[string]any)
...
func (cb *CoinBig) GetTicker(currencyPair CurrencyPair) (*Ticker, error) {
	...
	data := bodyDataMap["data"].(map[string]any)
	tickerMap := data["ticker"].(map[string]any)
	var ticker Ticker
	ticker.Date = num.ToInt[uint64](data["date"])
	ticker.Last = num.ToFloat64(tickerMap["last"])
	ticker.Buy = num.ToFloat64(tickerMap["buy"])
	ticker.Sell = num.ToFloat64(tickerMap["sell"])
	ticker.Low = num.ToFloat64(tickerMap["low"])
	ticker.High = num.ToFloat64(tickerMap["high"])
	ticker.Vol = num.ToFloat64(tickerMap["vol"])
	return &ticker, nil
}
func (cb *CoinBig) GetServerSync() error {
	...
	data, _ := bodyDataMap["data"].(map[string]any)
	stime := int64(num.ToInt[int](data["time"]))
