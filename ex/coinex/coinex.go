...
func (coinex *CoinEx) GetTicker(currency CurrencyPair) (*Ticker, error) {
	...
	tickermap := datamap["ticker"].(map[string]any)
	return &Ticker{
		Date: num.ToInt[uint64](datamap["date"]) / 1000,
		Last: num.ToFloat64(tickermap["last"]),
		Buy:  num.ToFloat64(tickermap["buy"]),
		Sell: num.ToFloat64(tickermap["sell"]),
		High: num.ToFloat64(tickermap["high"]),
		Low:  num.ToFloat64(tickermap["low"]),
		Vol:  num.ToFloat64(tickermap["vol"])}, nil
}
...
func (coinex *CoinEx) GetAccount() (*Account, error) {
	...
	for c, v := range datamap {
		...
		acc.SubAccounts[currency] = SubAccount{
			...
			ForzenAmount: num.ToFloat64(vv["frozen"])}
	}
	return acc, nil
}
...
func (coinex *CoinEx) doRequestInner(method, uri string, params *url.Values) (buf []byte, err error) {
	...
	if "POST" == method {
		...
		var parammap = make(map[string]string, 2)
	...
	return NewRequest(coinex.httpClient, method, reqUrl, paramStr, headermap)
}
func (coinex *CoinEx) doRequest(method, uri string, params *url.Values) (map[string]any, error) {
	resp, err := coinex.doRequestInner(method, uri, params)
	if err != nil {
		return nil, err
	}
	retmap := make(map[string]any, 1)
	err = json.Unmarshal(resp, &retmap)
	if err != nil {
		return nil, err
	}
	if num.ToInt[int](retmap["code"]) != 0 {
...
func (coinex *CoinEx) adaptTradeStatus(status string) TradeStatus {
	var tradeStatus = ORDER_UNFINISH
	switch status {
	case "not_deal":
...
func (coinex *CoinEx) adaptOrder(ordermap map[string]any, pair CurrencyPair) Order {
	return Order{
		Currency:   pair,
		OrderID:    num.ToInt[int](ordermap["id"]),
		OrderID2:   fmt.Sprint(num.ToInt[int](ordermap["id"])),
		Amount:     num.ToFloat64(ordermap["amount"]),
		Price:      num.ToFloat64(ordermap["price"]),
		DealAmount: num.ToFloat64(ordermap["deal_amount"]),
		AvgPrice:   num.ToFloat64(ordermap["avg_price"]),
		Status:     coinex.adaptTradeStatus(ordermap["status"].(string)),
		Side:       coinex.adaptTradeSide(ordermap["type"].(string)),
		Fee:        num.ToFloat64(ordermap["deal_fee"]),
		OrderTime:  num.ToInt[int](ordermap["create_time"])}
}
