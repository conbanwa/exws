...
func (bfx *Bitfinex) String() string {
	return BITFINEX
}
func (bfx *Bitfinex) GetTicker(currencyPair CurrencyPair) (*Ticker, error) {
	...
	ticker.Last = num.ToFloat64(resp["last_price"])
	ticker.Vol = num.ToFloat64(resp["volume"])
	ticker.High = num.ToFloat64(resp["high"])
	ticker.Low = num.ToFloat64(resp["low"])
	ticker.Sell = num.ToFloat64(resp["ask"])
	ticker.Buy = num.ToFloat64(resp["bid"])
...
func (bfx *Bitfinex) GetDepth(size int, currencyPair CurrencyPair) (*Depth, error) {
	...
	for _, bid := range bids {
		_bid := bid.(map[string]any)
		amount := num.ToFloat64(_bid["amount"])
		price := num.ToFloat64(_bid["price"])
		dr := DepthRecord{Amount: amount, Price: price}
		depth.BidList = append(depth.BidList, dr)
	}
	for _, ask := range asks {
		_ask := ask.(map[string]any)
		amount := num.ToFloat64(_ask["amount"])
		price := num.ToFloat64(_ask["price"])
		dr := DepthRecord{Amount: amount, Price: price}
...
func (bfx *Bitfinex) GetKlineRecords(currencyPair CurrencyPair, period KlinePeriod, size int, optional ...OptionalParameter) ([]Kline, error) {
	...
	respRaw, err := NewRequest(bfx.httpClient, "GET", apiURL, "", nil)
	if err != nil {
		return nil, err
	}
	var candles []Kline
...
func (bfx *Bitfinex) placeOrder(orderType, side, amount, price string, pair CurrencyPair) (*Order, error) {
	...
	order := new(Order)
	order.Currency = pair
	order.OrderID = num.ToInt[int](respMap["id"])
	order.OrderID2 = fmt.Sprint(num.ToInt[int](respMap["id"]))
	order.Amount = num.ToFloat64(amount)
	order.Price = num.ToFloat64(price)
	order.AvgPrice = num.ToFloat64(respMap["avg_execution_price"])
	order.DealAmount = num.ToFloat64(respMap["executed_amount"])
...
func (bfx *Bitfinex) CancelOrder(orderId string, currencyPair CurrencyPair) (bool, error) {
	var respMap map[string]any
	path := "order/cancel"
	err := bfx.doAuthenticatedRequest("POST", path, map[string]any{"order_id": num.ToInt[int](orderId)}, &respMap)
	if err != nil {
		return false, err
	}
	return respMap["is_cancelled"].(bool), nil
}
func (bfx *Bitfinex) toOrder(respMap map[string]any) *Order {
	order := new(Order)
	order.Currency = symbolToCurrencyPair(respMap["symbol"].(string))
	order.OrderID = num.ToInt[int](respMap["id"])
	order.OrderID2 = fmt.Sprint(num.ToInt[int](respMap["id"]))
	order.Amount = num.ToFloat64(respMap["original_amount"])
	order.Price = num.ToFloat64(respMap["price"])
	order.DealAmount = num.ToFloat64(respMap["executed_amount"])
	order.AvgPrice = num.ToFloat64(respMap["avg_execution_price"])
...
func (bfx *Bitfinex) GetUnfinishedOrders(currencyPair CurrencyPair) ([]Order, error) {
	var ordersmap []any
	err := bfx.doAuthenticatedRequest("POST", "orders", map[string]any{}, &ordersmap)
	if err != nil {
		return nil, err
	}
	var orders []Order
...
func (bfx *Bitfinex) doAuthenticatedRequest(method, path string, payload map[string]any, ret any) error {
	...
	resp, err := NewRequest(bfx.httpClient, method, apiURLV1+"/"+path, "", map[string]string{
...
func klineFromRaw(pair CurrencyPair, raw []any) *Kline {
	return &Kline{
		Pair:      pair,
		Timestamp: num.ToInt[int64](raw[0]),
		Open:      num.ToFloat64(raw[1]),
		Close:     num.ToFloat64(raw[2]),
		High:      num.ToFloat64(raw[3]),
		Low:       num.ToFloat64(raw[4]),
		Vol:       num.ToFloat64(raw[5]),
	}
}
