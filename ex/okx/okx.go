...
func (ok *OKX) ExchangeName() string {
	return OKEX
}
func (ok *OKX) SetCustomCID(f func() string) {
	ok.customCIDFunc = f
}
...
// OPTION：期权
//
//	func (ok *OKX) GetTickersV5(instType, uly string) ([]Ticker, error) {
...
func (ok *OKX) GetTickerV5(instId string) (*TickerV5, error) {
...
func (ok *OKX) GetDepthV5(instId string, size int) (*DepthV5, error) {
...
func (ok *OKX) GetKlineRecordsV5(instId string, period KlinePeriod, params *url.Values) ([][]string, error) {
...
func (ok *OKX) BuildRequestBody(params any) (string, *bytes.Reader, error) {
	if params == nil {
...
func (ok *OKX) doParamSign(httpMethod, uri, requestBody string) (string, string) {
	timestamp := IsoTime()
...
func (ok *OKX) DoAuthorRequest(httpMethod, uri, reqBody string, response any) error {
	endpoint := ok.config.Endpoint + uri
	sign, timestamp := ok.doParamSign(httpMethod, uri, reqBody)
	//logs.D("timestamp=", timestamp, ", sign=", sign)
	resp, err := NewRequest(ok.config.HttpClient, httpMethod, endpoint, reqBody, map[string]string{
...
func (ok *OKX) CreateOrder(param *CreateOrderParam) (*OrderSummaryV5, error) {
...
func (ok *OKX) CancelOrderV5(instId, ordId, clOrdId string) (*OrderSummaryV5, error) {
	reqBody := make(map[string]any)
...
func (ok *OKX) ClosePositions(instId, mgnMode, posSide string) (*OrderSummaryV5, error) {
	reqBody := make(map[string]any)
	reqBody["instId"] = instId
...
func (ok *OKX) GetPendingOrders(param *PendingOrderParam) ([]OrderV5, error) {
...
func (ok *OKX) GetOrderV5(instId, ordId, clOrdId string) (*OrderV5, error) {
	reqBody := make(map[string]string)
	reqBody["instId"] = instId
	if ordId != "" {
		reqBody["ordId"] = ordId
...
func (ok *OKX) GetOrderHistory(instType, instId, ordType, state, afterID, beforeID string) ([]OrderV5, error) {
	reqBody := make(map[string]string)
...
func (ok *OKX) GetAssetBalances(currency string) ([]AssetSummary, error) {
	reqBody := make(map[string]any)
...
func (ok *OKX) GetAccountBalances(currency string) (*BalanceV5, error) {
	reqBody := make(map[string]any)
...
func (ok *OKX) adaptKLineBar(period KlinePeriod) string {
	bar := "1D"
	switch period {
