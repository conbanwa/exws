...
func (okV3Ws *OKExV3FuturesWs) getContractAliasAndCurrencyPairFromInstrumentId(instrumentId string) (alias string, pair CurrencyPair) {
	if strings.HasSuffix(instrumentId, "SWAP") {
		...
		if err != nil {
			logs.E("instrument id invalid:", err)
			return "", UNKNOWN_PAIR
		}
	...
	case "candle":
		...
		for _, t := range klineResponse {
			...
			okV3Ws.klineCallback(&FutureKline{
				Kline: &Kline{
					Pair:      pair,
					High:      num.ToFloat64(t.Candle[2]),
					Low:       num.ToFloat64(t.Candle[3]),
					Timestamp: ts.Unix(),
					Open:      num.ToFloat64(t.Candle[1]),
					Close:     num.ToFloat64(t.Candle[4]),
					Vol:       num.ToFloat64(t.Candle[5]),
				},
				Vol2: num.ToFloat64(t.Candle[6]),
			}, 1)
		}
		return nil
	case "depth5":
