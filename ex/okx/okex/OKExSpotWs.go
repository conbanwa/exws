...
func (okV3Ws *OKExV3SpotWs) BBOCallback(bboCallback func(*Bbo)) {
	okV3Ws.bboCallback = bboCallback
}
...
func (okV3Ws *OKExV3SpotWs) SubscribeDepth(currencyPair CurrencyPair) error {
	...
	return okV3Ws.v3Ws.Subscribe(map[string]any{
		"op":   "subscribe",
...
func (okV3Ws *OKExV3SpotWs) SubscribeBBO(sm []string) (err error) {
	if okV3Ws.bboCallback == nil {
		return errors.New("please set ticker callback func")
	}
	for sym := range sm {
		if err = okV3Ws.v3Ws.Subscribe(map[string]any{
			"op":   "subscribe",
			"args": []string{fmt.Sprintf("spot/ticker:%v", sym)},
		}); err != nil {
			return err
		}
	}
...
func (okV3Ws *OKExV3SpotWs) handle(ch string, data json.RawMessage) error {
	...
	case "spot/trade":
		...
		for _, resp := range tradeResponse {
			...
			if err != nil {
				logs.W("parse timestamp error:", err)
			}
		...
		if strings.HasPrefix(ch, "spot/candle") {
			...
			for _, k := range candleResponse {
				...
				okV3Ws.klineCallback(&Kline{
					Pair:      pair,
					Timestamp: tm.Unix(),
					Open:      num.ToFloat64(k.Candle[1]),
					Close:     num.ToFloat64(k.Candle[4]),
					High:      num.ToFloat64(k.Candle[2]),
					Low:       num.ToFloat64(k.Candle[3]),
					Vol:       num.ToFloat64(k.Candle[5]),
				}, adaptSecondsToKlinePeriod(num.ToInt[int](periodMs)))
			}
			return nil
		}
	}
