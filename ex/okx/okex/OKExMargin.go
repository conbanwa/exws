...
func (ok *OKExMargin) GetMarginAccount(pair CurrencyPair) (*MarginAccount, error) {
	...
	acc.LiquidationPrice = num.ToFloat64(response["liquidation_price"])
	acc.RiskRate = num.ToFloat64(response["risk_rate"])
	acc.MarginRatio = num.ToFloat64(response["margin_ratio"])
	for k, v := range response {
		if strings.Contains(k, "currency") {
			...
			acc.Sub[c] = MarginSubAccount{
				Balance:     num.ToFloat64(vv["balance"]),
				Frozen:      num.ToFloat64(vv["frozen"]),
				Available:   num.ToFloat64(vv["available"]),
				CanWithdraw: num.ToFloat64(vv["can_withdraw"]),
				Loan:        num.ToFloat64(vv["borrowed"]),
				LendingFee:  num.ToFloat64(vv["lending_fee"])}
		}
	}
	return &acc, nil
}
...
func (ok *OKExMargin) Borrow(parameter BorrowParameter) (borrowId string, err error) {
	var param = struct {
		...
		Amount:       num.FloatToString(parameter.Amount, 8)}
...
func (ok *OKExMargin) Repayment(parameter RepaymentParameter) (repaymentId string, err error) {
	...
	param := struct {
		...
		num.FloatToString(parameter.Amount, 8)}
	reqBody, _, _ := ok.BuildRequestBody(param)
	println(reqBody)
	var response struct {
...
func (ok *OKExMargin) GetUnfinishedOrders(currency CurrencyPair) ([]Order, error) {
