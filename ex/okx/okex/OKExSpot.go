...
func (ok *OKExSpot) PlaceOrder(ty string, ord *Order) (*Order, error) {
	...
	if !response.Result {
		return nil, errors.New(int32(num.ToInt[int](response.ErrorCode)), response.ErrorMessage)
	}
...
func (ok *OKExSpot) adaptOrder(response OrderResponse) *Order {
	ordInfo := &Order{
		...
		Price:      num.ToFloat64(response.Price),
		Amount:     response.Size,
		AvgPrice:   num.ToFloat64(response.PriceAvg),
		DealAmount: num.ToFloat64(response.FilledSize),
		Status:     ok.adaptOrderState(response.State),
		Fee:        num.ToFloat64(response.Fee)}
	switch response.Side {
	case "buy":
		if response.Type == "market" {
			ordInfo.Side = BUY_MARKET
			ordInfo.DealAmount = num.ToFloat64(response.Notional) //成交金额
		} else {
			ordInfo.Side = BUY
		}
	case "sell":
		if response.Type == "market" {
			...
			ordInfo.DealAmount = num.ToFloat64(response.Notional) //成交数量
		} else {
			ordInfo.Side = SELL
		}
	}
	...
	if err != nil {
		logs.E("parse timestamp err=", err, ",timestamp=", response.Timestamp)
	} else {
...
func (ok *OKExSpot) GetUnfinishedOrders(currency CurrencyPair) ([]Order, error) {
...
func (ok *OKExSpot) String() string {
	return OKEX
}
...
type spotTickerResponse struct {
	...
	BidSize       float64 `json:"best_bid_size,string"`
	AskSize       float64 `json:"best_ask_size,string"`
...
func (ok *OKExSpot) GetDepth(size int, currency CurrencyPair) (*Depth, error) {
	...
	for _, itm := range response.Asks {
		dep.AskList = append(dep.AskList, DepthRecord{
			Price:  num.ToFloat64(itm[0]),
			Amount: num.ToFloat64(itm[1]),
		})
	}
...
func (ok *OKExSpot) GetKlineRecords(currency CurrencyPair, period KlinePeriod, size int, optional ...OptionalParameter) ([]Kline, error) {
	...
	optParam := url.Values{}
	util.MergeOptionalParameter(&optParam, optional...)
	...
	for _, itm := range response {
		...
		klines = append(klines, Kline{
			Timestamp: t.Unix(),
			Pair:      currency,
			Open:      num.ToFloat64(itm[1]),
			High:      num.ToFloat64(itm[2]),
			Low:       num.ToFloat64(itm[3]),
			Close:     num.ToFloat64(itm[4]),
			Vol:       num.ToFloat64(itm[5])})
	}
	return klines, nil
}
...
func (ok *OKExSpot) GetTrades(currencyPair CurrencyPair, since int64) ([]Trade, error) {
	...
	for _, item := range response {
		...
		trades = append(trades, Trade{
			Tid:    item.TradeId,
			Type:   util.AdaptTradeSide(item.Side),
			Amount: item.Size,
			Price:  item.Price,
			Date:   t.Unix(),
			Pair:   currencyPair,
...
func (ok *OKExSpot) GetCurrenciesPrecision() ([]OKExSpotSymbol, error) {
	...
	for _, v := range response {
		...
		if len(pres) == 1 {
			...
			sym.PricePrecision = len(pres[1])
		}
		pres = strings.Split(v.SizeIncrement, ".")
		if len(pres) == 1 {
			sym.AmountPrecision = 0
		} else {
			sym.AmountPrecision = len(pres[1])
		}
